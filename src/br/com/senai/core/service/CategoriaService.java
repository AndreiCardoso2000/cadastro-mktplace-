...
public class CategoriaService {
	...
	public Categoria buscarPor(int id) {
		if(id > 0) {
			...
			if (categoriaEncontrada == null) {
				throw new IllegalArgumentException("Categoria não encontrada no código informado");
			}
	...
	public List<Categoria> listarPor(String nome) {
		...
		throw new IllegalArgumentException("O filtro é obrigatório e deve conter mais de 3 caracteres");
	}
	...
	private void validar(Categoria categoria) {
		if (categoria != null) {
			 ...
			 if (isNomeInvalido) {
				 throw new IllegalArgumentException("O nome da categoria é obrigatório e não deve possuir "
